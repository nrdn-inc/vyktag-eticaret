...
// Çalıştırmak için: npm run test:db (bkz. vitest.db.config.ts)
//
// Proje ayrı bir test veritabanı kullanmıyor. Sipariş oluşturma stok düştüğü için,
// testler gerçek ürünlerin stoğunu tüketmesin diye vitrinde GÖRÜNMEYEN
// (product.isActive = false) ancak sipariş verilebilir (variant.isActive = true) özel
// bir test varyantı kullanılır; stoğu her testten önce bilinen bir değere sıfırlanır.
